...
save_data_path = 'C:/Python_Programs/Fai_ER_Project/Peer_Revision_Work/Final Graph Edits 3/MFPT_S2_Network 10 original.xlsx'
image_path = 'C:/Python_Programs/Fai_ER_Project/12_27_S2Cell1460_Image/S2Cell1460_im10.png' #if you extracted network from an image you can use it as a nackround, if no image put None.
...
edge_path = 'C:/Python_Programs/Fai_ER_Project/11_21_S2Cell_Data/edges10_matlab_flip_v2.xls' #excel file containing adjecency matrix of network. Please look at examples in Github Repo for reference.
node_path = 'C:/Python_Programs/Fai_ER_Project/11_21_S2Cell_Data/verts10_matlab_flip_v2.xls'#excel file conatining x y coordinates of all nodes within network. Look at github repo for examples.
name = "S2 10 original" #name of network, will be used for naming files and such
...
save_graph_path = "C:/Python_Programs/Fai_ER_Project/Peer_Revision_Work/Final Graph Edits 3" #name of file you want to save graph to.
...
class Network:
    ...
    def MFPT_global_dream_SM_two_junc(self, all_target_inverses, a, b, step, edges_without_ab, edges_copy): #Named is honor of the professors dream, will return the MFPT of all nodes given a missing edge connection between a and b
        ...
        for i in range(len(edges_copy)):
            ...
            for j in range(len(edges_copy)-1):
                    n += occupancy[j,i]/(len(edges_copy))
            Ave_MFPT[i] += n
    ...
    def plotdet_edge_sm_two_junc_cheap(self, step, save_data_path): #provides a matrix where the i'th row will give the x and y co-ordinates of the midpoint of each edge, and the Ave_MFPT should that edge be removed. These are provided in each column respectively.
        ...
        plt.figure(dpi = 600)
        return B, all_inverses, ghost_list, new_points, path_list, edges_copy
    def MFPT_edge_removal_plot_two_junc(self, step, line_width, save_data_path, save_data, image_bounds, save_graph, save_graph_path, cmap, vmin, vmax):
        if save_data == True:
        ...
        for i, j in A[:,3:]:
            H.add_edge(i,j)
        MFPT_global = self.MFPT_global_cheap_two_junc(step, I, new_points, edges_copy)
        colors = A[:,2] - MFPT_global
        vmin_orig = min(colors)
        vmax_orig = max(colors)
        vmax = 28.6021879688233
        vmin = 0.38925754816547453
        ...
        if save_graph:
            ...
            plt.savefig(save_graph_path + f"/{self.name}_edge.pdf", format='pdf')
        plt.show()
        plt.clf()
...
cmaps = [plt.cm.rainbow]
for cmap in cmaps:
    network_dict[name].MFPT_edge_removal_plot_two_junc(0.1, line_width, save_data_path, save_data, image_bounds, save_graph, save_graph_path, cmap, 0, 100)
