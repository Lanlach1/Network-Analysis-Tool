...
save_data_path = 'C:/Python_Programs/Fai_ER_Project/Peer_Revision_Work/Final Graph Edits 2/MFPT_NMJ_Network 50 original.xlsx'
image_path = 'C:/Python_Programs/Fai_ER_Project/NMJ_middle2/NMJ_middle2_im50.png' #if you extracted network from an image you can use it as a nackround, if no image put None.
...
edge_path = 'C:/Python_Programs/Fai_ER_Project/NMJ_middle2/NMJ_middle2_edges50.xls' #excel file containing adjecency matrix of network. Please look at examples in Github Repo for reference.
node_path = 'C:/Python_Programs/Fai_ER_Project/NMJ_middle2/NMJ_middle2_verts50.xls'#excel file conatining x y coordinates of all nodes within network. Look at github repo for examples.
name = "NMJ 50 original" #name of network, will be used for naming files and such
...
class Network:
    ...
    def theory_stat(self, targets, step): #will copmute the fendmental matrix given target conditions and a step size
        ...
        return N
    def average_target_step_theory(self, step, image_bounds, save_graph, save_graph_path): #same as previous except non uniform  transistion probability
        ...
        for i in range(len(self.points)):
            ...
            for j in range(len(self.points)-1):
                    n += occupancy[j,i]/(len(self.points))
            Ave_MFPT[i] += n
        for i in range(len(Ave_MFPT)):
            if Ave_MFPT[i] >= 600:
                Ave_MFPT[i] = 600
            elif Ave_MFPT[i] <= 141.2959273888025:
                Ave_MFPT[i] = 141.2959273888025
        plt.figure(dpi = 1000)
        ...
        plt.axis('off')
        if save_graph:
            plt.savefig(str(save_graph_path) + f'/{self.name}_Node.pdf', format='pdf')
            plt.savefig(str(save_graph_path) + f'/{self.name}_Node.png', format='png')
        plt.show() 
        plt.rcdefaults()
...
network_dict[name].average_target_step_theory(0.1, image_bounds, save_graph, save_graph_path)