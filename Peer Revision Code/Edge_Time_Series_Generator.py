...
save_data_path = 'C:/Python_Programs/Fai_ER_Project/S2_Cell_All/S2_Cell_DATA'
image_path = 'C:/Python_Programs/Fai_ER_Project/S2_Cell_All/Image' #if you extracted network from an image you can use it as a nackround, if no image put None.
...
edge_path = 'C:/Python_Programs/Fai_ER_Project/S2_Cell_All/Edges' #excel file containing adjecency matrix of network. Please look at examples in Github Repo for reference.
node_path = 'C:/Python_Programs/Fai_ER_Project/S2_Cell_All/Verts'#excel file conatining x y coordinates of all nodes within network. Look at github repo for examples.
name = "S2 Cell" #name of network, will be used for naming files and such

#these setting below are for the figure being created.
...
save_graph_path = "C:/Python_Programs/Movie_File" #name of file you want to save graph to.
...
class Network:
    ...
    def MFPT_global_dream_SM_two_junc(self, all_target_inverses, a, b, step, edges_without_ab, edges_copy): #Named is honor of the professors dream, will return the MFPT of all nodes given a missing edge connection between a and b
        ...
        for i in range(len(edges_copy)):
            ...
            for j in range(len(edges_copy)-1):
                    n += occupancy[j,i]/(len(edges_copy)-1)
            Ave_MFPT[i] += n
        return np.average(Ave_MFPT)
    ...
    def plotdet_edge_sm_two_junc(self, step, save_data_path): #provides a matrix where the i'th row will give the x and y co-ordinates of the midpoint of each edge, and the Ave_MFPT should that edge be removed. These are provided in each column respectively.
            ...
            for j in range(len(edges_copy)):
                if D[j,i] == 1: #This checks for connection
                    ...
                    if self.zero_degree_check(edges_without_ab) is True and self.edge_seperation_check_two_junc(step, all_inverses[target_checker,:,:], i, j, target_checker, edges_without_ab, edges_copy) == 1:
                        A[i,j,:] = [(new_points[i,0]+new_points[j,0])/2, (new_points[i,1]+new_points[j,1])/2,  self.MFPT_global_dream_SM_two_junc(all_inverses, i, j, step, edges_without_ab, edges_copy), i, j]
                        D[i,j] = 1    
                    else:
    ...
    def MFPT_global_cheap_two_junc(self, step, all_inverses, new_points, edges_copy): #Named is honor of the professors dream, will return the MFPT of all nodes given no missing edges
        Ave_MFPT = np.zeros(len(edges_copy))
        occupancy = np.zeros((len(edges_copy)-1, len(edges_copy)))
        for i in range(len(edges_copy)):
        ...
        for i in range(len(edges_copy)):
            ...
            for j in range(len(edges_copy)-1):
                    n += occupancy[j,i]/(len(edges_copy)-1)
            Ave_MFPT[i] += n
        print(Ave_MFPT, np.average(Ave_MFPT), np.std(Ave_MFPT))
        return np.average(Ave_MFPT), np.std(Ave_MFPT)/math.sqrt(len(self.points))
    def plotdet_edge_sm_two_junc_cheap(self, step, save_data_path): #provides a matrix where the i'th row will give the x and y co-ordinates of the midpoint of each edge, and the Ave_MFPT should that edge be removed. These are provided in each column respectively.
        ...
        plt.figure(dpi = 600)
        return B, all_inverses, ghost_list, new_points, path_list, edges_copy
    def MFPT_edge_removal_plot_two_junc(self, step, line_width, save_data_path, save_data, image_bounds, save_graph, save_graph_path):
        if save_data == True:
            A, I, ghost_list, new_points, path_list, edges_copy = self.plotdet_edge_sm_two_junc(step, save_data_path)
        else:
        ...
        for i, j in A[:,3:]:
            H.add_edge(i,j)
        MFPT_global, MFPT_global_std = self.MFPT_global_cheap_two_junc(step, I, new_points, edges_copy)
        ...
        if D_check == 1:
            for i in range(len(D)):
                ...
                connectpoints_r(self.points[:,0],self.points[:,1], int(D[i,0][0]), int(D[i,0][1]), line_width)
        ...
        if save_graph:
            plt.savefig(save_graph_path + f"/{self.name}_edge.png", format='png')
        plt.show()
        plt.clf()
        return(MFPT_global, MFPT_global_std)
...
def connectpoints_r(x,y,p1,p2, width): #Will make a line between the two points in the next graph
    x1, x2 = x[p1], x[p2]
    y1, y2 = y[p1], y[p2]
    plt.plot([x1,x2],[y1,y2],'maroon', linewidth=width)
def ER_network(name, diction, image_path, edge_path, node_path, image_bounds):
    #plt.figure(dpi=1000)
    #plt.rcParams["figure.figsize"] = [13.00,10.50]
    #plt.rcParams["figure.autolayout"] = True
...
datasplit = []
frame_number = []
for i in os.listdir(save_data_path):
    number = ''
    for c in i:
        if c.isdigit():
            number = number + c
    name_file = name + f'{number}'
    image_path_file = image_path + f'/S2cell1460_im{number}.png'
    node_path_file = node_path + f'/verts{number}_matlab_flip_v2.xls'
    edge_path_file = edge_path + f'/edges{number}_matlab_flip_v2.xls'
    save_data_path_file = save_data_path + f'/MFPT_ER_Network {number}.xlsx'
    ER_network(name_file, network_dict, image_path_file, edge_path_file, node_path_file, image_bounds)
    TAGMFPT.append(network_dict[name_file].MFPT_edge_removal_plot_two_junc(0.1, line_width, save_data_path_file, save_data, image_bounds, save_graph, save_graph_path))
    frame_number.append(int(number))
zipped = zip(TAGMFPT, frame_number)
data = sorted(zipped, key=lambda x: x[1])
print(data)
time = []
for n, i in enumerate(data):
    datasplit.append(i[0])
    TAGMFPT[n] = datasplit[n][0]
    std.append(datasplit[n][1]) 
    frame_number[n] = i[1]
    time.append(frame_number[n]/10)
plt.clf()
fig, ax = plt.subplots()
ax.errorbar(time, TAGMFPT, yerr=std,fmt='-o')
font = {'family': 'serif',
        'color':  'black',
        'weight': 'normal',
        'size': 25,
...
plt.ylabel("TAGMFPT", fontdict = font)
plt.ylim(3750, 6000)
