...
save_graph_path = "C:/Python_Programs/Movie_File" #path for saving figure. Will save in both PDF and PNG form
...
class Network:
    ...
    def plotdet_edge_sm_two_junc_cheap(self, step, save_data_path): #provides a matrix where the i'th row will give the x and y co-ordinates of the midpoint of each edge, and the Ave_MFPT should that edge be removed. These are provided in each column respectively.
        ...
        plt.figure(dpi = 600)
        return B, all_inverses, ghost_list, new_points, path_list, edges_copy
    def MFPT_edge_removal_plot_two_junc(self, step, line_width, save_data_path, save_data, image_bounds, save_graph, save_graph_path, colorbar_limits):
        if save_data == True:
...
network_dict[name].MFPT_edge_removal_plot_two_junc(0.1, line_width, save_data_path, save_data, image_bounds, save_graph, save_graph_path, colorbar_limits)
