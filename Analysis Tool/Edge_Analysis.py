...
save_data_path = 'C:/Python_Programs/Fai_ER_Project/Example Networks/Neuromuscular Junction/MFPT.xlsx'
...
line_width = 1 #with of the lines connecting juctins, will need to be adjusted for different sized networks. 
save_graph = True #if you want to save graph set to True
save_graph_path = "C:/Users/Owner/Downloads" #path for saving figure. Will save in both PDF and PNG form
colorbar_limits = [-10, 100]
figure_dpi = 200
...
class Network:
    ...
    def plotdet_edge_sm_two_junc_cheap(self, step, save_data_path): #provides a matrix where the i'th row will give the x and y co-ordinates of the midpoint of each edge, and the Ave_MFPT should that edge be removed. These are provided in each column respectively.
        ...
        plt.figure(dpi = 600)
        return B, all_inverses, ghost_list, new_points, path_list, edges_copy
    def MFPT_edge_removal_plot_two_junc(self, step, line_width, save_data_path, save_data, image_bounds, save_graph, save_graph_path, colorbar_limits, figure_dpi):
        if save_data == True:
        ...
        plt.figure(figsize=(8, 6), dpi=figure_dpi)
        if self.im is None:
            pass
        else:           
        ...
        for i in range(len(colors_corrected)): #this just sets a limit for the colorbar
            if colors_corrected[i] >= colorbar_limits[1]:
                colors_corrected[i] = colorbar_limits[1]
        #print(posi)
        nx.draw(G, posi, node_color='black', node_size=0, edge_color=colors_corrected, width=line_width, edge_cmap=cmap,
           with_labels=False, vmin=colorbar_limits[0], vmax=colorbar_limits[1])
        #print(self.points)
        #plt.scatter(A[:,0], A[:,1], cmap = 'plasma', c = A[:,2] - self.MFPT_global(step))
        sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin = colorbar_limits[0], vmax=colorbar_limits[1]))
        sm._A = []
        CL = plt.colorbar(sm)
        ...
        if save_graph:
            plt.savefig(str(save_graph_path) + f'/{self.name}_edge_ana.pdf', format='pdf')
            plt.savefig(str(save_graph_path) + f'/{self.name}_edge_ana.png', format='png')
        plt.show()
        plt.clf()
...
network_dict[name].MFPT_edge_removal_plot_two_junc(0.1, line_width, save_data_path, save_data, image_bounds, save_graph, save_graph_path, colorbar_limits, figure_dpi)
